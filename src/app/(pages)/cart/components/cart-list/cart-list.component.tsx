...
export const CartList = () => {
  ...
  return <div className="overflow-y-auto h-36 max-[400px]:h-28 sm:h-auto max overflow-x-hidden">
    {cart.map((item: IBoardGame, index: number) => (
      <div key={index} className="flex items-center justify-between mb-4 max-[400px]:px-3 py-3 break-words">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center justify-start flex-row">
            ...
            <div className="flex flex-col">
              <h2 className="text-sm sm:text-lg font-semibold w-full">{item.name}</h2>
              <p className="text-xs sm:text-base text-gray-600 w-full">{item.status}</p>
