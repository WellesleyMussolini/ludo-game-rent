...
export const PrimaryButton = ({ isLoading, text, type, disabled, handleClick }: IPrimaryButton) => <button
    ...
    ${type === EnumPrimaryButton.OUTLINED && "text-primary transparent border-2 border-primary hover:bg-primary hover:text-white cursor-pointer uppercase"}
...
`} disabled={disabled} onClick={handleClick}>
    {type === EnumPrimaryButton.GOOGLE
        ?
        (isLoading ? <LoadingSpinner size={30} color="primary" /> : <div className="flex items-center justify-center gap-3"><GoogleIcon /> {text}</div>)
        :
        (isLoading ? <LoadingSpinner size={30} color="primary" /> : text)}
</button>;