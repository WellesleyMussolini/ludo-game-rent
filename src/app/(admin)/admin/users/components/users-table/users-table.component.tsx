...
export const UsersTable = async ({ allUsers }: { allUsers: Array<IUser> }) => {
    return <div className="w-9/12 xl:max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="p-4 text-base font-medium text-gray-500 border-b border-gray-200 hidden md:flex md:justify-between bg-gray-100">
            ...
            allUsers.map((user: IUser, index: number) => {
                const { selectedRole, handleRoleChange } = useUserTable(user.id, user.role)
                return <div key={index} className="flex items-center px-4 py-2 border-b border-gray-200 hover:bg-gray-100 group">
                    ...
                    <div className="flex-grow md:flex md:justify-between md:items-center overflow-hidden overflow-ellipsis whitespace-nowrap">
                        <p className="text-xs xs:text-sm font-medium text-gray-900">{user.name}</p>
                        <p className="text-xs xs:text-sm text-gray-600 md:text-center">{user.email}</p>
                        <select
