...
import { createBoardGame } from "@/services/create-boardgame.service";
import { BoardGame } from "@/app/common/types/boardgame.types";
import { BoardGameFormAnimation, BoardGameFormSteps } from "../types/boardgame-form.types";
import { boardgameStatus } from "../mock/boardgame-status.mock";

export const useBoardgameForm = (handleVisibility: (visibility: boolean) => void) => {
    ...
    const [formState, setFormState] = React.useState<{ animation: string, formStep: string }>({
        animation: BoardGameFormAnimation.ANIMATION_JUMP_IN,
        formStep: BoardGameFormSteps.SEARCH_ID_STEP,
    });

    const [formStatus, setFormStatus] = React.useState<{ isLoading: boolean, dropdown: boolean }>({
        isLoading: false,
        dropdown: false,
    });
    ...
    const handleNextStep = async () => {
        ...
        try {
            ...
            setFormState({ ...formState, formStep: BoardGameFormSteps.SAVE_GAME_FORM_STEP });
        } catch {
    ...
    const handleCloseForm = () => {
        setFormState({ ...formState, animation: BoardGameFormAnimation.ANIMATION_JUMP_OUT });
        setTimeout(() => {
            setFormState({ ...formState, animation: BoardGameFormAnimation.ANIMATION_JUMP_IN });
    ...
    const handleOnChangeFields = React.useCallback((field: string, event: React.ChangeEvent<HTMLInputElement> | string) => {
        typeof event === "string"
