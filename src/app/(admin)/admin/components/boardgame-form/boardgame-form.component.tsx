...
import { BoardGameFormSteps, IBoardGameStepForm } from "./types/boardgame-form.types";
...
export const BoardGameForm = ({ visibility, handleVisibility }: IBoardGameStepForm) => {
    const {
        handleCloseForm,
        isSmallHeight,
        gameApiData,
    ...
    return visibility &&
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50">
            ...
            <div className={`
                ...
                ease-in-out       
                scroll-smooth         
                ${isSmallHeight && formState.formStep === BoardGameFormSteps.SAVE_GAME_FORM_STEP ? 'h-[90%]' : 'h-auto'
                }
            `}>
                {formState.formStep === BoardGameFormSteps.SEARCH_ID_STEP && (
                    <>

                        <div className={`z-10 absolute top-2 right-2 text-gray-500 hover:text-gray-600 cursor-pointer`}
                        ...
                        <div className="w-full">
                            <PrimaryButton
                                ...
                                type={!gameApiData.id ? PrimaryButtonStyles.DISABLED : PrimaryButtonStyles.OUTLINED}
                            />
                        </div>
                    </>
                )}
                ...
                {formState.formStep === BoardGameFormSteps.SAVE_GAME_FORM_STEP && (
                    <>
                        ...
                        <button className="absolute top-4 left-4 text-gray-500 hover:text-gray-700" onClick={handleReturnPreviousStep}>
                            <MdKeyboardDoubleArrowLeft size={25} />
                        </button>
                        <button className="absolute top-4 right-4 text-gray-500 hover:text-gray-700" onClick={handleCloseForm}>
                            <IoMdClose size={25} />
                        </button>
                        ...
                        <div className="w-full mb-6">
                            ...
                            <div className="relative">
                                <div
                                    className={`flex justify-between items-center border border-gray-300 rounded px-4 py-2 cursor-pointer ${formStatus.dropdown ? 'bg-gray-100' : 'bg-white'}`}
                                    onClick={() => setFormStatus({ ...formStatus, dropdown: !formStatus.dropdown })}
                                >
                                    <p className="font-medium text-gray-700">{gameApiData.status ?? "Selecione o status"}</p>
                                    <div>{formStatus.dropdown ? <IoIosArrowUp /> : <IoIosArrowDown />}</div>
                                </div>

                                {/* Dropdown Content */}
                                {formStatus.dropdown && (
                                    <div className="absolute w-full bg-white border border-gray-300 rounded mt-2 shadow-lg z-10">
                                        <Dropdown styles="w-full" visibility={formStatus.dropdown} content={dropdownContent} />
                                    </div>
                                )}
                            </div>
                        </div>
